        ...
        @if(!$services->isEmpty())
            @foreach($services as $service)
                <div class="border border-gray-600 bg-gray-600 text-white p-4 flex flex-col justify-between rounded-md">
                    ...
                    @if(!$tenant)
                        <a href="{{ route('services.pay', $service) }}" class="text-center px-6 py-2 w-full bg-blue-500 text-white rounded hover:bg-blue-600 transition duration-300">
                            Apply
                        </a>
                    @endif
                    @if($tenant)
                        <a href="{{route('services.update-view',[$tenant,$service])}}" class="text-center px-6 py-2 w-full bg-blue-500 text-white rounded hover:bg-blue-600 transition duration-300">
                            Apply
                        </a>
                    @endif
                </div>
